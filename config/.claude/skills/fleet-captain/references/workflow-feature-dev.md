...
Run the full test suite, typecheck, and build. Every check must pass before proceeding. Do not trust that resolve kept things green — verify fresh.

After verification passes, ship it:
Commit all changes, push to branch worktree-{{name}}, and create a PR to main. After pushing, run `gh pr checks <number> --watch` to block until all checks complete. If any check fails, fix and push again. Do not go idle until all checks pass.
...
After resolving, verify everything still works:
Run the full test suite, typecheck, and build. Every check must pass before proceeding.
...
| `context_warning` | Session compacting, may lose coherence | Watch for repeated actions. If degraded: recommend killing and respawning at current phase with `--branch` |
| `idle` after PR created | Pipeline complete | Verify PR exists, bind with `fleet desc --pr`, recommend killing the session |
