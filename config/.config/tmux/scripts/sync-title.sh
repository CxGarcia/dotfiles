#!/usr/bin/env bash
# sync-title.sh — Rename tmux session to match Claude Code's pane title (once)
...
# Only rename once per session
[[ "$(tmux show-options -sv -t "$SESSION" @title_synced 2>/dev/null)" == "1" ]] && exit 0
...
tmux set-option -s -t "$final" @title_synced 1 2>/dev/null
